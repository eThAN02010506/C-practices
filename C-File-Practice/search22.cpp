...
typedef void (*fn_free)(void *data);
...
void destoryDlink(DoubleLink *dlink)
{
    DoubleLinkNode *dln = dlink->tail;
    while (dlink->head != NULL)
    {
        if (dln->prev != NULL)
            {
                DoubleLinkNode *prev = dln->prev;
...
int hashDestroy(HashTable *ht, fn_free free_data)
{
...
int main(int argc, char *arrgv[])
{
    ...
    HashTable ht_p;
    Person *p11 = new_person("C", 19);
    initHashTable(&ht_p, 100);
    printf("%d\r\n ",hashInsert(&ht_p, p11->name, p11));
    Person *p12 = new_person("C", 19);
    if (hashInsert(&ht_p, p12->name, p12) == 0)
    {
        printf("0\r\n");
        free_person(p12);
    }

    DoubleLinkNode *dln = hashFind(&ht_p, "C", NULL);
    Person *p13 = (Person*)dln->data;

    printf("%s %d\r\n ",p13->name, p13->age);

    p11 = (Person*)hashDelete(&ht_p, "C");
    if (p11 != NULL)
    {
        printf("1\r\n");
        free(p11);
